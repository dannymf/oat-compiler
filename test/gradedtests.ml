...
let pass_all_executed_ll_file tests =
  List.map
    (fun (fn, ans) ->
      ( fn
      , fun () ->
          try assert_eqfs (fun () -> exec_ll_file fn "") ans () with
          | exn ->
            pass_all := false;
            raise exn ))
    tests
...
let pass_all_executed_oat_file tests =
  List.map
    (fun (path, args, ans) ->
      ( path ^ " args: " ^ args
      , fun () ->
          try assert_eqfs (fun () -> oat_file_e2e_test path args) ans () with
          | exn ->
            pass_all := false;
            raise exn ))
    tests
...
let assert_quality fn ll_ast =
  if not !pass_all
  then failwith "Your register allocator failed a correctness test"
  else (
    let _ = Opt.do_opt := true in
    let ll_ast = Opt.optimize ll_ast in
    let h_greedy, size_greedy, x86_greedy =
      compile_with_config "dataflow" "greedy" ll_ast
    in
    let h_better, size_better, x86_better =
      compile_with_config "dataflow" "better" ll_ast
    in
    let mem_greedy = Registers.memop_of_prog x86_greedy in
    let mem_better = Registers.memop_of_prog x86_better in
    let _ =
      if !Driver.print_regs_flag
      then
        Printf.printf
          "greedy sz: %4d mem: %4d\t\tbetter sz: %4d mem: %4d \t diff_sz: %4d diff_mem: \
           %4d - %s\n"
          size_greedy
          mem_greedy
          size_better
          mem_better
          (size_greedy - size_better)
          (mem_greedy - mem_better)
          fn
    in
    if mem_better < mem_greedy
    then ()
    else if size_better < size_greedy
    then ()
...
let throw_key_diff compare val_to_string a b =
  ...
  then ()
    ...
    failwith
    @@ Printf.sprintf
         "Output differs at labels: %s in maps\n%s\n%s\n"
         (String.concat ", " keys)
         str_a
         str_b)
;;
...
let ll_opt_file_test path optimize ans =
  ...
  let printer k b =
    Printf.sprintf "%s %s" (Lbl.to_string k) (Llutil.string_of_block tdecls b)
  in
...
let dfa_liveness_file (tests : (string * 'a Datastructures.LblM.t) list) =
  ...
  List.map
    (fun (path, ans) ->
      ( "liveness: " ^ path
      , fun () -> ll_dfa_file_test path (throw_key_diff Fact.compare printer) analyze ans
      ))
    tests
;;
...
let arithmetic_tests =
  ...
  ; ( "llprograms/sub_neg.ll"
    , "255" (* Why, oh why, does the termianl only report the last byte? *) )
...
let oat_correctness_tests =
  oat_regalloc_quality_tests @ hw3_globals_tests @ greedy_is_good_tests
;;
...
let hw3sp24_tests =
  ...
  ; "hw3programs/sp24_tests/balanced.oat", "\"(\"", "Not balanced1"
  ; ( "hw3programs/sp24_tests/laderman_mul3x3.oat"
    , ""
    , "A5 7 12 A\nA35 17 52 A\nA248 369 617 A0" )
  ; "hw3programs/sp24_tests/vector_clocks.oat", "", "1"
  ; ( "hw3programs/sp24_tests/graph_connectivity.oat"
    , "4 1 2 0 3 2 3"
    , "0001\n0010\n0101\n1010\nconnected: true0" )
  ; ( "hw3programs/sp24_tests/graph_connectivity.oat"
    , "4 1 2 0 3"
...
let tests : suite =
  [ GradedTest
      ("solver / liveness analysis tests", 10, dfa_liveness_file liveness_analysis_tests)
  ...
  ; GradedTest
      ("constprop analysis tests", 15, dfa_constprop_file constprop_analysis_tests)
